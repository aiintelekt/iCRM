...
pdfFileName = parameters.pdfFileName;
...
if (!invoice && invoiceId)    {
    ...
    try {
		if (UtilValidate.isNotEmpty(pdfFileName)) {
			UtilHttp.setContentDisposition(response, pdfFileName + ".pdf");
		}else {
...
//if (logoImageUrl) {
logoImageUrl = org.groupfio.common.portal.util.DataUtil.getLogoFilePath(delegator);
//logoImageUrl = "/bootstrap/images/esolutions-big.png";
...
applicationInternalUrl = org.fio.homeapps.util.DataUtil.getGlobalValue(delegator, "APPLICATION_INTL_URL");
context.put("applicationInternalUrl", applicationInternalUrl);
Debug.log("logoImageUrl======= "+context);
