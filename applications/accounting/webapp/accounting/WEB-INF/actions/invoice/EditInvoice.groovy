...
if (invoice) {
	...
	if (invoiceType) {
		context.invoiceType = invoiceType;
	}
	...
	if (billToParty) {
		  partyExter = invoice.getRelated("InvoiceItem", null, ["invoiceItemSeqId"], false);
	}
	...
	outstandingAmount = InvoiceWorker.getInvoiceNotApplied(invoice).multiply(conversionRate).setScale(decimals, rounding);
    context.invoiceTotal = invoiceTotal;
    context.invoiceNoTaxTotal = invoiceNoTaxTotal;
	context.outstandingAmount = outstandingAmount;
	...
	if(billToParty) {
	billToPartyId = billToParty.get("partyId");
	GenericValue partyIdentification = EntityQuery.use(delegator).from("PartyIdentification").where("partyId", billToPartyId,"partyIdentificationTypeId","ALT_DEAL_CUST_ID").queryOne();
	if(partyIdentification) {
	context.partyIdentificationId = partyIdentification.get("idValue");
	}else {
		context.partyIdentificationId = billToPartyId;
		}
		billToPartyName = PartyHelper.getPartyName(delegator, billToPartyId, false);
	context.billToPartyName = billToPartyName;
	}
    ...
    String hostedInvoiceUrl = org.groupfio.common.portal.util.UtilAttribute.getAttrValue(delegator, "InvoiceAttribute", "invoiceId", invoiceId, "hosted_invoice_url");
   	context.put("hostedInvoiceUrl", hostedInvoiceUrl);             
}