    ...
    public static boolean checkPriceCondition(GenericValue productPriceCond, String productId, String virtualProductId, String prodCatalogId,
        ...
        if ("PRIP_PRODUCT_ID".equals(productPriceCond.getString("inputParamEnumId"))) {
            ...
            if (UtilValidate.isEmpty(partyId)) {
                ...
                // and from/thru date within range
                List<GenericValue> customFieldPartyClassificationList = EntityQuery.use(delegator).from("CustomFieldPartyClassification").where("partyId", partyId, "customFieldId", partyClassificationGroupId).cache(true).filterByDate(nowTimestamp).queryList();

                // then 0 (equals), otherwise 1 (not equals)
                if (UtilValidate.isNotEmpty(customFieldPartyClassificationList)) {
                    compare = 0;
                } else {
                    compare = 1;
                }
