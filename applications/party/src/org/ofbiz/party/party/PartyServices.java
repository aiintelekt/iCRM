...
public class PartyServices {
    ...
    public static Map<String, Object> setPartyStatus(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            if (!statusId.equals(oldStatusId)) {
                ...
                if (statusId.equals("LEAD_CONVERTED")) {
                	statusId="PARTY_ENABLED";
                	party.set("statusId", statusId);
                }else{
                	party.set("statusId", statusId);
                ...
                /*if (statusId.equals("LEAD_EXPIRED")) {
            		Map<String, Object> inContext = new HashMap<String, Object>();
            		Map<String, Object> tmpResult = FastMap.newInstance();
            		 try {
            			 inContext.put("partyId", partyId);
    						tmpResult=dispatcher.runSync("crmsfa.deactivateAccount", inContext);
    						Debug.log("==============tmpResult==================="+tmpResult);
    					} catch (GenericServiceException e) {
    						String errMsg = "Problem deactivateAccount: " + e.toString();
    ...
    public static Map<String, Object> linkParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            attrsToMove = EntityQuery.use(delegator).from("PartyIdentification").where("partyId", partyId).queryList();
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return ServiceUtil.returnError(e.getMessage());
        }
        ...
        for (GenericValue attr: attrsToMove) {
            attr.set("partyId", partyIdTo);
            try {
                    if (EntityQuery.use(delegator).from("PartyIdentification").where(attr.getPrimaryKey()).queryOne() == null) {
                    attr.create();
                }
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
        ...
        try {
            delegator.removeByAnd("PartyIdentification", UtilMisc.toMap("partyId", partyId));
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return ServiceUtil.returnError(e.getMessage());
        }
        ...
        partyIdentification.set("partyIdentificationTypeId", "LINKED_TO");
        partyIdentification.set("idValue", partyIdTo);
        try {
            delegator.create(partyIdentification);
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return ServiceUtil.returnError(e.getMessage());
        }
