...
public class HtmlMenuWrapper {
    ...
    public static HtmlMenuWrapper getMenuWrapper(HttpServletRequest request, HttpServletResponse response, HttpSession session, String menuDefFile, String menuName, String menuWrapperClassName) {
        ...
        if (menuWrapper == null) {
            ...
            Map<String, Object> sanitizedParameterMap = new HashMap<>();
            for (Map.Entry<String, Object> entry : parameterMap.entrySet()) {
                String sanitizedKey = sanitizeInput(entry.getKey());
                Object sanitizedValue = sanitizeInput(entry.getValue());
                sanitizedParameterMap.put(sanitizedKey, sanitizedValue);
            }
            
            menuWrapper.setParameters(sanitizedParameterMap);
    ...
    public static String sanitizeInput(Object input) {
        if (input instanceof String) {
            return StringEscapeUtils.escapeHtml((String) input);  // Escapes HTML special characters
        }
        return ""+input;  // Return other types unchanged, or add additional handling as needed
    }
}
