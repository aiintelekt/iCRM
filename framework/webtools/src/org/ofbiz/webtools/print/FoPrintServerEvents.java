...
public class FoPrintServerEvents {
    ...
    public static String getXslFo(HttpServletRequest req, HttpServletResponse resp) {
        ...
        if (UtilValidate.isEmpty(screenUri) || !isValidScreenUri(screenUri)) {
            try {
                resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid screen URI");
            } catch (IOException e) {
    ...
    private static boolean isValidScreenUri(String screenUri) {
        if (screenUri == null || screenUri.isEmpty()) {
            return false;
        }
        try {
            URL url = new URL(screenUri);
