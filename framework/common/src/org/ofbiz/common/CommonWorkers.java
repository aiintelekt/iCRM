...
public class CommonWorkers {
    ...
    public static String getZipCodeRegex(Delegator delegator, String country) {
        if (UtilValidate.isEmpty(country)) {
            // Load the system default country
            country = EntityUtilProperties.getPropertyValue("general", "country.geo.id.default", delegator);
        }
        ...
        String regex = "";
        try {
        	GenericValue geo = EntityQuery.use(delegator)
                                      .from("Geo")
                                      .where("geoId", country, "geoTypeId", "COUNTRY")
                                      .queryOne();
        	regex = UtilValidate.isNotEmpty(geo.get("regex")) ? geo.get("regex").toString() : "";
        } catch (GenericEntityException e){
            Debug.logError(e, "Cannot lookup Geo", module);
        }
