...
public abstract class AbstractJmsListener implements GenericMessageListener, ExceptionListener {
    ...
    public void onMessage(Message message) {
        ...
        if (message instanceof MapMessage) {
            ...
            try {
				Debug.logInfo("MESSAGE IS " +txtMessage.getText(), module);
			} catch (JMSException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				
			}
            runService(txtMessage);
        } else {
    ...
    private Map<String, Object> runService(TextMessage txtMessage) {
        ...
        try {
        	...
        	Destination destination = txtMessage.getJMSDestination();
        	serviceName = destination.toString();
        	serviceName = serviceName.replaceAll("queue://", "");
        	textMessage = txtMessage.getText();
        	
        	
        	//Store In table
        	GenericValue jmsLogInfo = delegator.makeValue("JmsLogInfo");
        	String jmsMessageId = delegator.getNextSeqId("JmsLogInfo");
        	jmsLogInfo.set("jmsMessageId", jmsMessageId);
        	jmsLogInfo.set("message", textMessage);
        	jmsLogInfo.set("serviceName", serviceName);
        	jmsLogInfo.create();
        	
        	//System.out.println(" Delegator is " + delegator);
        	LocalDispatcher localdispatcher = ServiceContainer.getLocalDispatcher("entity-" + delegator.getDelegatorName(), delegator);
            ModelService model = localdispatcher.getDispatchContext().getModelService(serviceName);
            
            if (context != null) {
                try {
                	context.put("jmsMessageId", jmsMessageId);
                	localdispatcher.runAsync(serviceName, context);
                } catch (GenericServiceException gse) {
                    Debug.logError(gse, "Problems with service invocation.", module);
                }
            }
