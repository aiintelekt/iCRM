...
public class ContainerLoader implements StartupLoader {
    ...
    public static synchronized Container loadContainers(String config, String[] args) throws StartupException {
    	//System.out.println("Container loadContainers calling from ContextFilter...... loaded >> "+loaded);
        if (!loaded) {
            ContainerLoader loader = new ContainerLoader();
            
            //Start.getInstance().main(new String[0]);
            
            //Config cfg = Start.getInstance().getConfig();
            
            Config cfg = Start.getInstance().config;
            
            // start.config = 
            
            if (UtilValidate.isEmpty(cfg)) {
            	try {
					cfg = new Config(new String[0]);
					Start.getInstance().config = cfg;
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					System.out.println("Container loadContainers exception during load configuration......  >> ");
				}
            }
