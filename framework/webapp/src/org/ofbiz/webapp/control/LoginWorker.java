...
public class LoginWorker {
    ...
    public static String getExternalLoginKey(HttpServletRequest request) {
		...
		externalKey = (String) session.getAttribute(EXTERNAL_LOGIN_KEY_ATTR);
        if (externalKey != null){
    ...
    public static GenericValue checkLogout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin != null) {
            ...
            Delegator delegator = (Delegator) request.getAttribute("delegator");
          //check the access of the component and return error
            if(UtilValidate.isNotEmpty(userLogin) && UtilValidate.isNotEmpty(delegator) && !"XMLHttpRequest".equals(request.getHeader("X-Requested-With"))) {
        		try {
        			boolean hasComponentPermission = org.ofbiz.webapp.control.LoginWorker.hasComponentAccess(userLogin, delegator, request);
        			if(!hasComponentPermission) {
        				
        				Debug.logInfo("User does not have permission to access this module", module);
                        userLogin = null;

                        // have to reget this because the old session object will be invalid
                        session = request.getSession();
                        session.setAttribute("_LOCKED_USER_", null);
                        request.setAttribute("_ERROR_MESSAGE_", "User does not have permission to access this module");
    ...
    public static String checkLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin == null) {        	
          ...
          //double check the component access
        	Delegator delegator = (Delegator) request.getAttribute("delegator");
        	userLogin = (GenericValue) request.getSession(true).getAttribute("userLogin");
        	if(UtilValidate.isNotEmpty(userLogin) && UtilValidate.isNotEmpty(delegator) && !"XMLHttpRequest".equals(request.getHeader("X-Requested-With"))) {
        		try {
        			boolean hasComponentPermission = org.ofbiz.webapp.control.LoginWorker.hasComponentAccess(userLogin, delegator, request);
        			if(!hasComponentPermission) {
    ...
    public static String login(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (ModelService.RESPOND_SUCCESS.equals(result.get(ModelService.RESPONSE_MESSAGE))) {
            ...
            String roleTypeId = (String) result.get("roleTypeId");
            if(UtilValidate.isNotEmpty(roleTypeId))
            	session.setAttribute("_USER_PERMISSION_ROLE_", roleTypeId);
    ...
    public static void doBasicLogout(GenericValue userLogin, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (userLogin != null) {
            ...
            session.setAttribute("_USER_PERMISSION_ROLE_", null);
        }
	...
	public static boolean hasComponentAccess(GenericValue userLogin, Delegator delegator, HttpServletRequest request){
    	Delegator delegator1 = (Delegator) request.getAttribute("delegator");
    	if(UtilValidate.isNotEmpty(delegator1))
    		...
    		if(UtilValidate.isNotEmpty(userLogin)) {
        		...
        		if(!hasPermission) {
        			...
        			if(UtilValidate.isEmpty(groupIds)) {
        				Map<String, Object> result = getUserRoleGroup(delegator, userLoginPartyId);
        			...
        			if(UtilValidate.isNotEmpty(groupIds)) {
        				...
        				EntityCondition condition = EntityCondition.makeCondition("groupId",EntityOperator.IN,groupIds);
        				List<GenericValue> securityGrpPermList = EntityQuery.use(delegator).from("SecurityGroupPermission").where(condition).queryList();
        				List<String> permissionIds = UtilValidate.isNotEmpty(securityGrpPermList) ? EntityUtil.getFieldListFromEntityList(securityGrpPermList, "permissionId", true) : new ArrayList<String>() ;
        				
        				if(UtilValidate.isNotEmpty(permissionIds)) {
        					List<GenericValue> componentAccess = EntityQuery.use(delegator).select("componentName","description","requestUri","permissionId").from("OfbizComponentAccess").where(EntityCondition.makeCondition("permissionId", EntityOperator.IN, permissionIds)).queryList();
        					components = UtilValidate.isNotEmpty(componentAccess) ? EntityUtil.getFieldListFromEntityList(componentAccess, "componentName", true) : new ArrayList<String>();
        				}
        				String contextPath = request.getContextPath();
        				...
        				Debug.logInfo("components---->"+components+"---contextPath --->"+contextPath, module);
        			}
        		}
    		}
    	} catch (Exception e) {
    ...
    public static Map<String, Object> getUserRoleGroup(Delegator delegator, String userLoginPartyId){
    	...
    	try {
    		...
    		String menuParentRoleType = EntityUtilProperties.getPropertyValue("general", "SECURITY_ROLE", delegator);
        	DynamicViewEntity dynamicViewEntity = new DynamicViewEntity();
        	dynamicViewEntity.addMemberEntity("PTY", "Party");
        	dynamicViewEntity.addAlias("PTY", "partyId");
        	...
        	EntityCondition condition1 = EntityCondition.makeCondition(EntityOperator.AND,
        			...
        			EntityCondition.makeCondition("parentTypeId", EntityOperator.EQUALS, UtilValidate.isNotEmpty(menuParentRoleType) ? menuParentRoleType : "SECURITY_ROLE")
        			);
    		List<GenericValue> partyRoles = EntityQuery.use(delegator).from(dynamicViewEntity).where(condition1).queryList();
