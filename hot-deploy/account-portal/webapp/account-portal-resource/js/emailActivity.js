
$(function() {
	var typeId  = $("#srTypeId").val();
    if (typeId != "") {
    	loadSubTypes(typeId);
    }
	$("#srTypeId").change(function() {
       var typeId  = $("#srTypeId").val();
        if (typeId != "") {
            loadSubTypes(typeId);
        }
    });
    $("#emailTemplate_desc").change(function() {
        var templateId = $("#emailTemplate_val").val();
        loadTemplate(templateId);
    });

});

function loadSubTypes(typeId) {
        var nonSelectContent = "<span class='nonselect'>Please Select</span>";
        var subTypes = '<option value="" >Please Select</option>';
        $.ajax({
            type: "POST",
            url: "getIASubTypes",
            data: { "iaTypeId": typeId },
            async: false,
            success: function(data) {
                   var sourceDesc = data.results;
                   for (var i = 0; i < data.length; i++) {
                        var type = data[i];
                        if(type.subTypeId === '${selectedSubTypeId!}'){
                        	subTypes += '<option value="'+type.subTypeId+'" selected>'+type.subTypeDesc+'</option>';
                        }else{
                        	subTypes += '<option value="'+type.subTypeId+'">'+type.subTypeDesc+'</option>';
                        }
                    }
            }
        });
        $("#srSubTypeId").html(DOMPurify.sanitize(subTypes));
}

function loadTemplate(templateId){
    var template = "";
    $.ajax({
        type: "POST",
        url: "loadTemplate",
        data: { "templateId": templateId },
        async: false,
        success: function(result) {
            template = result[0].template;
            //console.log("template-----result------"+result+"---------"+template);
            $("#emailFormContent").remove();
            $("#mainFrom").append("<input type='hidden' id='emailFormContent' name='emailFormContent' value='"+templateId+"' /> ");
            var editor = CKEDITOR.instances['emailContent'];
            editor.setData(template);
            //$("#emailContent").html($.parseHTML(template));
           // $("#emailContent").html(template);
           // alert("parse-----===================================------------"+$.parseHTML(template));
           //$("#emlContent").html("<textarea id='emailContent' class='ckeditor' value='"+template+"'></textarea>");
           //$("#emailContent").html("<@textareaLarge label='Messages' id='emailFormContent' rows='10' value='"+template+"'/>");
           //$("#emailFormContent").html(""<@textareaLarge label='Messages' id='emailFormContent' rows='10' value=template/>);        
        }
    });
    
}

