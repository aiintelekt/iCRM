...
<script>
$('#teamMemberModal').on('click', '.parentSet', function(){
    var value = $(this).children("span").attr("value");
    $('#leadAssignTo').val(value);
   		...
   		"language": {
            ...
            "columns": [
               { "data": "partyId",
   	            ...
   	            data = '<a href="#" class="parentSet"><span id="parAccId1_'+row.id+'" name="parAccId_'+row.id+'" value ="' + data + '"></span>'+row.name+'('+data+')</a>';
   	            return data;
   	         }
   	      	},
