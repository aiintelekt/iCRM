...
if (UtilValidate.isNotEmpty(partyId)) {
    conditionPR = EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("partyIdFrom", EntityOperator.EQUALS, partyId),
	...
	responsibleFor = EntityQuery.use(delegator).from("PartyRelationship").where(conditionPR).orderBy("fromDate DESC").queryFirst();
	...
	if (UtilValidate.isNotEmpty(responsibleFor)) {
	    ...
	    /*
	    personRep = delegator.findOne("Person", UtilMisc.toMap("partyId", partyIdTo), false);
	    if (UtilValidate.isNotEmpty(personRep)) {
	        personResponsible = personRep.getString("firstName") + " " + personRep.getString("lastName");
	    }
	    */
