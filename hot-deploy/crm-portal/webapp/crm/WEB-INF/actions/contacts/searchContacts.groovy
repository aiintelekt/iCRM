...
if (UtilValidate.isNotEmpty(searchFirstName)) {
	EntityCondition nameCondition =  EntityCondition.makeCondition(UtilMisc.toList(EntityCondition.makeCondition("firstName", EntityOperator.LIKE, "%"+searchFirstName+"%"),
...
if(UtilValidate.isEmpty(accountSearchPartyId) && UtilValidate.isEmpty(searchFirstName) && UtilValidate.isEmpty(searchEmailId) && UtilValidate.isEmpty(searchPhoneNum)) {
	...
	if (partyIdList != null && partyIdList.size() > 0) {
		...
		for (String partyId: partyIdList) {
			...
			if (PartySummaryDetailsViewGv != null && PartySummaryDetailsViewGv.size() > 0) {
				...
				String name = PartySummaryDetailsViewGv.getString("firstName")+" "+PartySummaryDetailsViewGv.getString("lastName");
