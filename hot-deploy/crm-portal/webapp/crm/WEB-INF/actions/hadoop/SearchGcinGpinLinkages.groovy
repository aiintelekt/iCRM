...
if (UtilValidate.isNotEmpty(searchPartyId)) {
	GenericValue customerSme = EntityUtil.getFirst( delegator.findByAnd("HdpCustomerSme", UtilMisc.toMap("lcin", searchPartyId), null, false) );
	if (UtilValidate.isNotEmpty(customerSme)) {
		EntityCondition partyCondition = EntityCondition.makeCondition("gcin", EntityOperator.EQUALS, customerSme.getString("gcin"));
		conditionsList.add(partyCondition);
	}
}
...
if (UtilValidate.isNotEmpty(identification)) {
	String lcin = identification.getString("idValue");
	GenericValue customerSme = EntityUtil.getFirst( delegator.findByAnd("HdpCustomerSme", UtilMisc.toMap("lcin", lcin), null, false) );
	if (UtilValidate.isNotEmpty(customerSme)) {
		EntityCondition partyCondition = EntityCondition.makeCondition("gcin", EntityOperator.EQUALS, customerSme.getString("gcin"));
		conditionsList.add(partyCondition);
	}
} else {
	EntityCondition partyCondition = EntityCondition.makeCondition("gcin", EntityOperator.EQUALS, null);
	conditionsList.add(partyCondition);
}
*/
...
count = delegator.findCountByCondition("HdpGcinGpinLinkage", mainConditons, null, UtilMisc.toSet("gcin"), efoNum);

/*
entityList = delegator.findList("HdpGcinGpinLinkage", mainConditons, UtilMisc.toSet("gcin"), null, efo, false);
...
entityList = delegator.findList("HdpGcinGpinLinkage", mainConditons, null, null, efo, false);
