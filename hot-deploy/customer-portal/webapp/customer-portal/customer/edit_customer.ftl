...
<#include "component://loyalty-portal/webapp/loyalty-portal/picker/picker/picker.ftl"/>
         ...
         <form id="mainForm" method="post" action="<@ofbizUrl>editCustomerAction</@ofbizUrl>" data-toggle="validator"> 
        ...
        <#--  <@inputHidden id="postalCode" value="${partySummary.primaryPostalCode?if_exists}"/>-->
      	...
      	<@pageSectionHeader title="Edit Customer Information" id="alignPageHeader" extra='' />
		<#else>
		<@pageSectionHeader title="Edit Customer Information" id="alignPageHeader" extra='<div class="float-right">
		     	 <p> <span>${helpUrl?if_exists}</span> </p></div>' />
		</#if>
           ...
           <h4 class="bg-light pl-1 mt-2">Personal Information </h4> 
			...
			<h4 class="bg-light pl-1 mt-2">Contact Information </h4>
             ...
             <div id="submitEditModal" class="modal fade" role="dialog">
...
$(document).ready(function() {
		...
		if (countryGeoId != null && countryGeoId != "") {
			...
			$.ajax({
				...
				success: function(states) {
					$('[id="mainForm"] #editStateProvinceGeoId').empty();
					list = $('[id="mainForm"] #editStateProvinceGeoId');
			...
			$('[id="mainForm"] #editStateProvinceGeoId').append(list);
			$('[id="mainForm"] #editStateProvinceGeoId').dropdown('refresh');
		}
	});
	...
	$('#editCountryGeoId').change(function(e, data) {
		$('[id="mainForm"] #editStateProvinceGeoId').dropdown("clear");
		...
		if (countryGeoId != '') {
			...
			$('[id="mainForm"] #editStateProvinceGeoId').html('<option value="">Please Select</option>');
		}
	});
