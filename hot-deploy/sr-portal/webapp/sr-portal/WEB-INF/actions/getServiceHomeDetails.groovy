...
long onHoldSrCount = 0;
long receivedSrCount = 0;
...
if(UtilValidate.isNotEmpty(userLogin)) {
	...
	if(rs !=null){
		try{
			...
			while (rs.next()) {
				...
				} else if("on_hold".equals(barType)){
					onHoldSrCount = Long.valueOf(elementcount);
				} else if("received".equals(barType)){
					receivedSrCount = Long.valueOf(elementcount);
				}
	...
	dashboardBarContext.put("sr-on-hold", onHoldSrCount);
	dashboardBarContext.put("sr-received", receivedSrCount);
	...
	if(UtilValidate.isNotEmpty(csrRoles) && !csrRoles.contains(roleTypeId)) {
		...
		condList.add(EntityCondition.makeCondition("productStoreGroupTypeId", EntityOperator.EQUALS, "PSGT_AREA"));
		
		EntityCondition cond = EntityCondition.makeCondition(condList, EntityOperator.AND);
		List<GenericValue> storeGroupList = EntityQuery.use(delegator).select("productStoreGroupId","productStoreGroupName").from("ProductStoreGroup").where(cond).queryList();
		context.put("storeGroupList", storeGroupList);
		
		Map<String, List> locList = new LinkedHashMap();
		for (GenericValue storeGroup : storeGroupList) {
			conditions.add(EntityCondition.makeCondition("primaryStoreGroupId", EntityOperator.EQUALS, storeGroup.getString("productStoreGroupId")));
			EntityCondition condition = EntityCondition.makeCondition(conditions, EntityOperator.AND);
			List<GenericValue> locationList = EntityQuery.use(delegator).select("productStoreId","storeName").from("ProductStore").where(condition).queryList();
			locList.put(storeGroup.getString("productStoreGroupId"), locationList);
		}
		context.put("locList", locList);
