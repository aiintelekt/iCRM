...
<div class="row">
  ...
  <#--Changes made to add back button in update activitiy for LEAD,ACCOUNT,CONTACT,OPPOURNITY-->
       ...
       <#if screen_Name?if_exists == "CUSTOMER">
  ...
  <#--end-->
      	<form method="post" action="<#if (parameters.srNumber)?has_content>updateActivityEvent<#else>updateActivityEvent</#if>" id="mainFrom" class="form-horizontal" name="mainFrom" novalidate="novalidate" data-toggle="validator" onsubmit="return submitActivityForm();">
       		<input type="hidden" name="custRequestId" value="${(parameters.srNumber)!}"/>
                	<div class="col-md-12 col-md-6 col-sm-12">
                    	...
                    	<@inputHidden name="contactId" id="contactId" value = "${inputContext.contactId!}"/>
...
function loadContacts(){
	...
	var partyId = $("#cNo").val();
		$.ajax({
	        ...
	        data: {"partyId": partyId, "externalLoginKey": "${requestAttributes.externalLoginKey!}","isIncludeMainParty":"Y"},
	        async: false,
	        success: function (data) {   
	            if (data) {
