...
.create-email .dash-panel{
    ...
    <div class="row">
        <div id="main" role="main">
            ...
            <div class="dropdown-menu" aria-labelledby="E10007">
                ...
                <a class="dropdown-item" href="/contact-portal/control/addTask?partyId=${partyId?if_exists}&domainEntityType=${domainEntityType!}&domainEntityId=${domainEntityId!}&externalLoginKey=${requestAttributes.externalLoginKey!}" target="_blank"><i class="fa fa-clipboard" aria-hidden="true"></i> Task</a> 
			    <a class="dropdown-item" href="/contact-portal/control/addPhoneCall?partyId=${partyId?if_exists}&domainEntityType=${domainEntityType!}&domainEntityId=${domainEntityId!}&externalLoginKey=${requestAttributes.externalLoginKey!}" target="_blank"><i class="fa fa-phone" aria-hidden="true"></i> Phone Call</a>
			    ...
			    <a class="dropdown-item" href="/contact-portal/control/addAppointment?partyId=${partyId?if_exists}&domainEntityType=${domainEntityType!}&domainEntityId=${domainEntityId!}&externalLoginKey=${requestAttributes.externalLoginKey!}" target="_blank"><i class="fa fa-calendar" aria-hidden="true"></i> Appointment</a>
			    <a class="dropdown-item" href="/contact-portal/control/addOthers?partyId=${partyId?if_exists}&domainEntityType=${domainEntityType!}&domainEntityId=${domainEntityId!}&externalLoginKey=${requestAttributes.externalLoginKey!}" target="_blank"><i class="fa fa-plus-square" aria-hidden="true"></i> Others</a>
            </div>
            ' />
  ...
  <div class="col-lg-12 col-md-12 col-sm-12 dash-panel create-email">
            <@sectionFrameHeader   title="${uiLabelMap.addEmail}"    />
            <form method="post" action="<#if (parameters.salesOpportunityId)?has_content>addSalesEmailEvent<#else>addEmailEvent</#if>" id="SrEmail" class="form-horizontal" name="phone" novalidate="novalidate" data-toggle="validator" onsubmit="return submitEmailActivityForm();" enctype="multipart/form-data">
                ...
                <div>
		                ...
		                <@dynaScreen 
			                instanceId="CREATE_EMAIL_ACTIVITY_CONT"
			                modeOfAction="CREATE"
			             />
...
$(function() {
	...
	link.setAttribute('href', "/contact-portal/control/addEmail?partyId=" + partyId + "&domainEntityType=CONTACT&domainEntityId=" + partyId);
	var userName = $("#userName").val();
	$('#type').val($('#workEffortTypeId').val());
	$('#type').attr('readonly', 'readonly');
	$('#ownerBuDesc').attr('readonly', 'readonly');
...
function formSubmission() {
	...
	if ($('#srSubTypeId').val() == "") {
		showAlert('error', 'Please select SubType');
		valid = false;
	}
	if ($('#nto').val() == "") {
		showAlert('error', 'Please select TO Email');
		valid = false;
	}
