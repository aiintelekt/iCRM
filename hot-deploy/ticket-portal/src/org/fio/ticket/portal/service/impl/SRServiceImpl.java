package org.fio.ticket.portal.service.impl;
...
import org.fio.ticket.portal.service.impl.SRServiceImpl;
...
public class SRServiceImpl {
	...
	public static Map<String,Object> getSrCategory(DispatchContext dctx, Map context) {
		...
		try {
			...
			conditionList.add(EntityCondition.makeCondition("type", EntityOperator.EQUALS, "SRCategory"));
			if (UtilValidate.isNotEmpty(srTypeId)) {
				conditionList.add(EntityCondition.makeCondition("parentCode", EntityOperator.EQUALS, srTypeId));
			}
			conditionList.add(EntityCondition.makeCondition(EntityCondition.makeCondition("active",EntityOperator.EQUALS, null),
					EntityOperator.OR,
			...
			List < GenericValue > srCategorires = delegator.findList("CustRequestAssoc", mainCondition, null, UtilMisc.toList("sequenceNumber"), null, false);
			...
			if (UtilValidate.isNotEmpty(srCategorires)) {
				for (GenericValue srCategory: srCategorires) {
					...
					data.put("srCategoryId",srCategory.getString("code"));
					data.put("srCategoryDesc",srCategory.getString("value"));
					results.add(data);
				}
				result.put("results", results);
			}
	...
	public static Map<String,Object> getSrSubCategory(DispatchContext dctx, Map context) {
		...
		try {
			List conditionList = FastList.newInstance();
			if(UtilValidate.isNotEmpty(srCategoryId)) {
				conditionList.add(EntityCondition.makeCondition("type", EntityOperator.EQUALS, "SRSubCategory"));
				conditionList.add(EntityCondition.makeCondition("parentCode", EntityOperator.EQUALS, srCategoryId));
				conditionList.add(EntityCondition.makeCondition(EntityCondition.makeCondition("active",EntityOperator.EQUALS, null),
						EntityOperator.OR,
						EntityCondition.makeCondition("active", EntityOperator.EQUALS, "Y")));
				EntityCondition mainCondition = EntityCondition.makeCondition(conditionList, EntityOperator.AND);
				List < GenericValue > srSubCategorires = delegator.findList("CustRequestAssoc", mainCondition, null, UtilMisc.toList("sequenceNumber"), null, false);
				//List < GenericValue > srCategorires = EntityQuery.use(delegator).select("custRequestCategoryId","description").from("CustRequestCategory").where("parentCustRequestCategoryId", srCategoryId).orderBy("seqNum").queryList();
	...
	public static Map<String, Object> reassignSr(DispatchContext dctx, Map context) {
		...
		String externalId = (String) context.get("srNumber");
			...
			if(UtilValidate.isNotEmpty(accessLevel) && AccessLevel.YES.equals(accessLevel)) {
				...
				if (UtilValidate.isNotEmpty(emplTeam)) {    
					...
					if(UtilValidate.isNotEmpty(responsiblePersonExists) && responsiblePersonExists.equals(responsiblePerson)){
						...
						String data = "";
						data= "This "+externalId+"  is already assigned to the selected Owner";
						results.add(data);
				...
				if(UtilValidate.isNotEmpty(emplTeamId))
					updateCustRequest.put("emplTeamId", emplTeamId);*/
						...
						if (UtilValidate.isNotEmpty(externalId) && UtilValidate.isNotEmpty(userLoginId) && UtilValidate.isNotEmpty(previousOwnerId) && !previousOwnerId.equals(userLoginId)) {
							...
							String subject = "Assigned SR ID "+externalId+" - "+custRequestName;
					...
					String nsender = loggedInUserContactInformation.get("EmailAddress");*/
							...
							Map<String, String> ntoContactInformation = org.groupfio.common.portal.util.PartyPrimaryContactMechWorker.getPartyPrimaryContactMechValueMaps(delegator,ownerId,UtilMisc.toMap("isRetriveEmail", true),true);
							...
							if (UtilValidate.isNotEmpty(nsender) && UtilValidate.isNotEmpty(nto)){
								...
								strVar += "<td>:</td>";
								strVar += "<td style=\"font-family:Verdana;text-align:left;padding-left: 5px;\"><a target=\"_blank\" href="+appUrl+"/sr-portal/control/viewServiceRequest?srNumber="+externalId+"&externalLoginKey="+externalLoginKey+">"+externalId+"</a></td>";
								strVar += "</tr>";
								strVar += "<tr>";
		...
		try {
			if(UtilValidate.isNotEmpty(noteId)) {
				...
				if(UtilValidate.isNotEmpty(noteDetails)) {
					...
					data.put("createdStamp", UtilValidate.isNotEmpty(noteDetails.get("createdStamp")) ? UtilDateTime.timeStampToString(noteDetails.getTimestamp("createdStamp"), "dd/MM/yyyy HH:mm", TimeZone.getDefault(), null) : "");
					data.put("noteDateTime", UtilValidate.isNotEmpty(noteDetails.get("noteDateTime")) ? UtilDateTime.timeStampToString(noteDetails.getTimestamp("noteDateTime"), "dd/MM/yyyy HH:mm", TimeZone.getDefault(), null) : "");
					data.put("notePartyName",notePartyName);
					data.put("createdByName",createdByName);
					results.add(data);
					result.put("results", results);
