...
import org.groupfio.customfield.service.reader.Reader;
import org.groupfio.customfield.service.reader.ReaderFactory;
...
public class EntityWriter extends Writer {
	...
	private void writeEntity (Map<String, Object> context) throws Exception {
		...
		String entityFilePath = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, "dynamic.entity.path");
		String entityModelFilePath = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, "dynamic.entity.model.template.path");
		...
		if (ResponseUtils.isSuccess(readerResult)) {
			...
			if(UtilValidate.isNotEmpty(entityXml)) {
				...
				String dynamicEntityModelPath = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, "dynamic.entity.model.path");
				...
				if (UtilValidate.isNotEmpty(dynamicEntityModelPath)) {
					...
					String modelSqlPath = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, "dynamic.entity.sql.path");
					
					//String entityModelSql = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, "dynamic.entity.model.seg.sql.template");
					String entityModelSql = UtilProperties.getPropertyValue(CustomfieldServiceConstants.configResource, context.get("sqlTemplateKey").toString());
