...
public class AccountDataValidator implements Validator {
	...
	private static String MODULE = AccountDataValidator.class.getName();
	...
	public Map<String, Object> validate(Map<String, Object> context) {
		...
		try {
			...
			if (UtilValidate.isEmpty(data.get("accountId"))) {
				validate = false;
				message = "accountId is empty" + " [Row No:" + rowNumber + "]";
				validationMessage.put("accountId", message);
			}
			
			if (UtilValidate.isEmpty(data.get("accountName"))) {
				validate = false;
				message = "AccountName is empty" + " [Row No:" + rowNumber + "]";
				validationMessage.put("accountName", message);
			}
