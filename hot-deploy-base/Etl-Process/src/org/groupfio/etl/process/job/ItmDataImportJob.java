 ...
 *
 */
public class ItmDataImportJob extends Thread {

	public static String etlItmTableName = UtilProperties.getPropertyValue("Etl-Process", "ITM_TABLE");
	private static String MODULE = ItmDataImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to itm staging [main]...................size: " + ", time:"
					+ UtilDateTime.nowAsString(), MODULE);
			int totalProceedCount = 0;
			List<GenericValue> findJobs = EntityQuery.use(delegator).from("EtlPreProcessor").where("statusId", "CREATED", "etlTableName", etlItmTableName).queryList();

			Debug.log("++++++++++++++++++++ItmDataImportJob+++++++++++++++++++" + findJobs);
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importItms", inputNew);
					Debug.log("++++++++++++++++++++importItmData+++++++++++++++++success++" + result);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportItm", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportProductSupplementary = delegator.findList("DataImportItm",
			...
			Debug.logInfo("End storing to ITM staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);
		} catch (Throwable t) {
			Debug.logError("ItmDataImportJob Error: "+t.getMessage(), MODULE);
		}
	}
