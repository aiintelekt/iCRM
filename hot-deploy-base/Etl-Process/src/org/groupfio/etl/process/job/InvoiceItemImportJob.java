 ...
 *
 */
public class InvoiceItemImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "INVOICE_ITEM_TABLE");

	private static String MODULE = InvoiceItemImportJob.class.getName();
		...
		try {
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				...
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("DataImportInvoiceItem", inputNew);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportInvoiceItem", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportInvoiceItem = delegator.findList(
												"DataImportInvoiceItem", null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportInvoiceItem)) {
											for (GenericValue gv : dataImportInvoiceItem) {
												GenericValue dataExportInvoiceItem = delegator.makeValue(
														"DataExportInvoiceItem",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportInvoiceItem.set("sequenceId",
														delegator.getNextSeqId("DataExportCustomer"));
												dataExportInvoiceItem.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportInvoiceItem.create();
			...
			Debug.logError("InvoiceItemImportJob Error: "+t.getMessage(), MODULE);
		}
	}
