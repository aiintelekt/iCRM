 ...
 *
 */
public class OrderImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "ORDER_TABLE");

	private static String MODULE = OrderImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to Order staging [main]...................size: " + ", time:"
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importOrders", inputNew);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportOrderHeader", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> etlImportOrderFields = delegator.findList(
												"EtlImportOrderFields", null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(etlImportOrderFields)) {
											for (GenericValue gv : etlImportOrderFields) {
												GenericValue etlExportOrderFields = delegator.makeValue(
														"EtlExportOrderFields",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												etlExportOrderFields.set("sequenceId",
														delegator.getNextSeqId("EtlExportOrderFields"));
												etlExportOrderFields.setNonPKFields(gv);
												TransactionUtil.begin();
												etlExportOrderFields.create();
			...
			Debug.logInfo("End storing to Order staging [main]...................endPoint: " + totalProceedCount + ", time:"
					+ UtilDateTime.nowAsString(), MODULE);

		} catch (Throwable t) {
			Debug.logError("OrderImportJob Error: "+t.getMessage(), MODULE);
		}
	}
