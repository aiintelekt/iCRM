 ...
 *
 */
public class ContactImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "CONTACT_TABLE");

	private static String MODULE = ContactImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to Contact staging [main]...................size: " + ", time:"
			...
			Debug.log("++++++++++++++++++++ContactImportJob+++++++++++++++++++" + findJobs);
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importContacts", inputNew);
					Debug.log("++++++++++++++++++++importContacts+++++++++++++++++success++" + result);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportContact", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportContact = delegator.findList("DataImportContact",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportContact)) {
											for (GenericValue gv : dataImportContact) {
												GenericValue dataExportContact = delegator.makeValue(
														"DataExportContact",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportContact.set("sequenceId",
														delegator.getNextSeqId("DataExportContact"));
												dataExportContact.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportContact.create();
			...
			Debug.logInfo("End storing to Contact staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);
		} catch (Throwable t) {
			Debug.logError("ContactImportJob Error: "+t.getMessage(), MODULE);
		}
	}
