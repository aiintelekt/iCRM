 ...
 *
 */
public class ActivityDataImportJob extends Thread {

	public static String etlActivityTableName = UtilProperties.getPropertyValue("Etl-Process", "ACTIVITY_TABLE ");
	private static String MODULE = ActivityDataImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to activity staging [main]...................size: " + ", time:"
					+ UtilDateTime.nowAsString(), MODULE);
			int totalProceedCount = 0;
			List<GenericValue> findJobs = EntityQuery.use(delegator).from("EtlPreProcessor").where("statusId", "CREATED", "etlTableName", etlActivityTableName).queryList();

			Debug.log("++++++++++++++++++++ActivityDataImportJob+++++++++++++++++++" + findJobs);
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importActivity", inputNew);
					Debug.log("++++++++++++++++++++importActivity+++++++++++++++++success++" + result);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportActivity", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportActivity = delegator.findList("DataImportActivity",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportActivity)) {
											for (GenericValue gv : dataImportActivity) {
												GenericValue dataExportItm = delegator.makeValue(
														"DataExportActivity",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportItm.set("sequenceId",
														delegator.getNextSeqId("DataExportActivity"));
			...
			Debug.logInfo("End storing to ACTIVITY staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);
		} catch (Throwable t) {
			Debug.logError("ActivityDataImportJob Error: "+t.getMessage(), MODULE);
		}
	}
