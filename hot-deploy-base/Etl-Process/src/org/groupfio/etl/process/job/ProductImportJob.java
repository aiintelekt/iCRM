 ...
 *
 */
public class ProductImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "PRODUCT_TABLE");

	private static String MODULE = ProductImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to product staging [main]...................size: " + ", time:"
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				...
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importProducts", inputNew);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportProduct", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportProduct = delegator.findList("DataImportProduct",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportProduct)) {
											for (GenericValue gv : dataImportProduct) {
												GenericValue dataExportProduct = delegator.makeValue(
														"DataExportProduct",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportProduct.set("sequenceId",
														delegator.getNextSeqId("DataExportProduct"));
												dataExportProduct.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportProduct.create();
			...
			Debug.logInfo("End storing to Product staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);

		} catch (Throwable t) {

			Debug.logError("ProductImportJob Error: "+t.getMessage(), MODULE);
		}
	}
