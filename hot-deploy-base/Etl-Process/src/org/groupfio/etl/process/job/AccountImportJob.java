 ...
 *
 */
public class AccountImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "ACCOUNT_TABLE");

	private static String MODULE = AccountImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to Account staging [main]...................size: " + ", time:"
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importAccounts", inputNew);
					Debug.log("++++++++++++++++++++importAccounts+++++++++++++++++success++" + result);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportAccount", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportAccount = delegator.findList("DataImportAccount",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportAccount)) {
											for (GenericValue gv : dataImportAccount) {
												GenericValue dataExportAccount = delegator.makeValue(
														"DataExportAccount",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportAccount.set("sequenceId",
														delegator.getNextSeqId("DataExportAccount"));
												dataExportAccount.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportAccount.create();
			...
			Debug.logInfo("End storing to Account staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);

		} catch (Throwable t) {
			Debug.logWarning("AccountImportJob Throwable: "+t.getMessage(), MODULE);
