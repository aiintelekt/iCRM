 ...
 *
 */
public class CustomerImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "CUSTOMER_TABLE");

	private static String MODULE = CustomerImportJob.class.getName();
	...
	public void run() {
		try {
			Debug.logInfo("Start storing to Customer staging [main]...................size: " + ", time:"
			...
			Debug.log("++++++++++++++++++++CustomerImportJob+++++++++++++++++++" + findJobs);
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importCustomers", inputNew);
					Debug.log("++++++++++++++++++++importCustomers+++++++++++++++++success++" + result);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportCustomer", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportCustomer = delegator.findList("DataImportCustomer",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportCustomer)) {
											for (GenericValue gv : dataImportCustomer) {
												GenericValue dataExportCustomer = delegator.makeValue(
														"DataExportCustomer",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportCustomer.set("sequenceId",
														delegator.getNextSeqId("DataExportCustomer"));
												dataExportCustomer.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportCustomer.create();
			...
			Debug.logInfo("End storing to Customer staging [main]...................endPoint: " + totalProceedCount
					+ ", time:" + UtilDateTime.nowAsString(), MODULE);
		} catch (Throwable t) {
			Debug.logError("CustomerImportJob Error: "+t.getMessage(), MODULE);
		}
	}
