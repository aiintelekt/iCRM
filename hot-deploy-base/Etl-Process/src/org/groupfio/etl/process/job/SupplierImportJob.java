 ...
 *
 */
public class SupplierImportJob extends Thread {

	public static String etlTableName = UtilProperties.getPropertyValue("Etl-Process", "SUPPLIER_TABLE");

	private static String MODULE = SupplierImportJob.class.getName();
		...
		try {
			...
			if (UtilValidate.isNotEmpty(findJobs)) {
				for (GenericValue job : findJobs) {
					...
					Map<String, Object> result = dispatcher.runSync("importSuppliers", inputNew);
					...
					if (ServiceUtil.isSuccess(result)) {
						...
						EntityCondition codeCondition = EntityCondition.makeCondition(
								...
								EntityOperator.AND,
								EntityCondition.makeCondition("batchId", EntityOperator.EQUALS, batchId));
						long processCount = delegator.findCountByCondition("DataImportSupplier", codeCondition, null,
								null);
							...
							if (UtilValidate.isNotEmpty(etlModel)) {
								...
								if (UtilValidate.isNotEmpty(etlModel1)) {
									...
									if (UtilValidate.isNotEmpty(etlSourceTable)) {
										...
										List<GenericValue> dataImportSupplier = delegator.findList("DataImportSupplier",
												null, selectFields, null, null, false);
										if (UtilValidate.isNotEmpty(dataImportSupplier)) {
											for (GenericValue gv : dataImportSupplier) {
												GenericValue dataExportSupplier = delegator.makeValue(
														"DataExportSupplier",
														UtilMisc.toMap("modelId", list.getString("modelId")));
												dataExportSupplier.set("sequenceId",
														delegator.getNextSeqId("DataExportSupplier"));
												dataExportSupplier.setNonPKFields(gv);
												TransactionUtil.begin();
												dataExportSupplier.create();
			...
			Debug.logError("SupplierImportJob Error: "+t.getMessage(), MODULE);
		}
	}
