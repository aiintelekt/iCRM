...
<#-- used components [start] -->

<#macro generalInput label id name="" value="" labelClass="col-sm-4 field-text" hint="" placeholder="" iconClass="" inputType="text" dataError="" tooltip="" iconLeft=true disabled=false readonly=false required=false min="" max="" step="" maxlength="" minlength="" pattern="" mandatory=false onkeyup="" labelColSize="col-sm-4" inputColSize="col-sm-7" isMakerChange=false>
<div class="form-group row">
   ...
   <div class="${inputColSize}">
      <div class="input-icon <#if !iconLeft>right</#if>">
         <input type="${inputType}" class="form-control input-sm <#if tooltip?has_content>tooltips</#if> <#if inputType=="number"> no-spin</#if>" autocomplete="off"  value="${value!}"  id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" placeholder="${placeholder}" <#if disabled>disabled</#if> <#if readonly>readonly</#if> <#if required>required</#if> <#if dataError?has_content>data-error="${dataError}"</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if onkeyup?has_content>onkeyup="${onkeyup}"</#if> <#if inputType=="number" && min?has_content> min="${min}" </#if> minlength="${minlength!}" maxlength="${maxlength!}"  <#if inputType == "email">pattern="^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\])+\.([A-Za-z]{2,4})$" <#else><#if pattern?has_content>pattern="${pattern!}"</#if></#if> <#if step?has_content>step="${step!}"</#if> >
         <#if disabled>
...
<div class="form-group row">
	...
	<label class="col-md-4 field-text <#if required>text-danger</#if>" for="${id}">${label}</label>
   	<div class="col-md-7">
   		<label class="field-text input-sm">
   			<span id="${id}">
...
<#macro dateInput label id name="" value="" dateViewmode="" dateStartFrom="" dateEndTo="" inputSize="sm" tooltip="" required=false dateFormat="DD-MM-YYYY" disablePastDate=false disableFutureDate=false labelColSize="col-sm-4" inputColSize="col-sm-7" displayCurrentDate=false isMakerChange=false>
<div class="form-group row">
   <@labels label=label required=required labelColSize=labelColSize isMakerChange=isMakerChange/>
   <div class="${inputColSize}">
      	<div class="input-group date-input-${id!} <#if tooltip?has_content>tooltips</#if>" <#if tooltip?has_content>data-original-title="${tooltip}"</#if> >
			<input class="form-control input-${inputSize!}" placeholder="${dateFormat!}" type="text" id="${id!}" name="<#if !name?has_content>${id}<#else>${name}</#if>" data-date-format="${dateFormat}" <#if dateStartFrom?has_content>data-date-min-date="${dateStartFrom!}"</#if> <#if dateEndTo?has_content>data-date-max-date="${dateEndTo!}"</#if> <#if disablePastDate>data-date-min-date="${.now}"</#if> <#if dateViewmode?has_content>data-date-viewmode="${dateViewmode!}"</#if> value="<#if value?has_content>${value!}</#if>" <#if required>required</#if> >
...
<#macro simpleDateInput id name="" value="" placeholder="" dateViewmode="" dateStartFrom="" dateEndTo="" inputSize="sm" tooltip="" required=false dateFormat="DD-MM-YYYY" disablePastDate=false disableFutureDate=false displayCurrentDate=false>
<div class="input-group date-input-${id!} date <#if tooltip?has_content>tooltips</#if>" <#if tooltip?has_content>data-original-title="${tooltip}"</#if> >
	<input class="form-control input-${inputSize!}" placeholder="<#if placeholder?exists && placeholder?has_content>${placeholder!}<#else>${dateFormat!}</#if>" type="text" id="${id!}" name="<#if !name?has_content>${id}<#else>${name}</#if>" data-date-format="${dateFormat}" <#if dateStartFrom?has_content>data-date-min-date="${dateStartFrom!}"</#if> <#if dateEndTo?has_content>data-date-max-date="${dateEndTo!}"</#if> <#if disablePastDate>data-date-min-date="${.now}"</#if> <#if dateViewmode?has_content>data-date-viewmode="${dateViewmode!}"</#if> value="<#if value?has_content>${value!}</#if>" <#if required>required</#if> >
	<span class="input-group-addon">
	<span class="fa fa-calendar"></span> 
	</span>
</div>
...
<#macro radioInput label name options value="" radioInline=false labelColSize="col-sm-4" inputColSize="col-sm-7" required=false>
<div class="form-group row">
	<label class="${labelColSize} field-text">${label} <#if required><span class="text-danger">&#42;</span></#if></label>
   	<div class="${inputColSize}">
   		<#list options.entrySet() as entry>  
   		<label class="<#if radioInline> radio-inline <#else> radio-list </#if>">
   			<input type="radio" id="${entry.key}" name="${name}" value="${entry.key}" <#if value?has_content && entry.key == value>checked</#if>>
   			${entry.value!}
   		</label>		
   		</#list>
   </div>
...
<div class="form-group row">
   <label class="col-md-4 field-text">${label}</label>
   <div class="col-md-7">
...
<div class="tabbable-line">
   <ul class="nav nav-tabs">
      ...
      <li class="nav-item">
         <a class="nav-link <#if activeTabId == entry.key>active</#if>" href="#${entry.key}" data-toggle="tab">
         ${tab.get("tabName")} </a>
      </li>
      </#list>
   </ul>
...
<#macro fromCommonAction cancelUrl="" cancelLabel="Cancel" submitLabel="Save" iconClass="" clearId="" showCancelBtn=true showSubmitBtn=true showClearBtn=false style="" onclick="">
<div class="clearfix"> </div>
   <div class="form-group row">
      <div class="col-sm-12" <#if style?has_content>style="${style}"</#if> >
      	 <#if showSubmitBtn>
...
<#macro simpleInput id name="" value="" placeholder="" inputType="text" dataError="" tooltip="" disabled=false readonly=false required=false  min="" max="" maxlength="" step="">
<div class="form-group mr">
 	<input type="${inputType}" class="form-control input-sm <#if tooltip?has_content>tooltips</#if> " autocomplete="off"  value="${value!}"  id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" placeholder="${placeholder}" <#if disabled>disabled</#if> <#if readonly>readonly</#if> <#if required>required</#if> <#if dataError?has_content>data-error="${dataError}"</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if inputType=="number" && min?has_content> min="${min}" </#if> maxlength="${maxlength!}" <#if step?has_content>step="${step!}"</#if> >
 	<#if disabled>
 	<input type="hidden" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" value="${value!}"/>	
 	</#if>
 	<div class="help-block with-errors" id="${id}_error"></div>
...
<#macro simpleInputModal id name="" modalName="" value="" placeholder="" inputType="text" dataError="" tooltip="" disabled=false readonly=false required=false  min="" max="" maxlength="" step="">
...
<#macro simpleDropdownInput id options="" hint="" name="" value="" required=false disabled=false tooltip="" filter=false onchange =""  emptyText="Please Select" allowEmpty=false dataLiveSearch=true isMultiple=false isUseLabels=false isClearable=true class="" dataAttribute="">
<div class="form-group <#if class?has_content>${class}</#if> mr <#if required>has-error</#if> ${id} " ${dataAttribute!}>
  	<select class="ui dropdown <#if dataLiveSearch>search</#if> form-control <#if tooltip?has_content>tooltips</#if>" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" <#if required>required</#if> <#if disabled>disabled</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> onchange="${onchange!}" <#if isMultiple>multiple=""</#if> >
  	<#if allowEmpty>
...
clearable: <#if isClearable>true<#else>false</#if>
...
<div class="form-group row">
   <label class="col-sm-6 field-text <#if required>text-danger</#if>" for="${id}">${label}</label>
   <div class="col-sm-5">
      <div class="input-icon <#if !iconLeft>right</#if>">
...
<div class="form-group row">
   <label class="col-md-4 field-text" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-md-7">
      <div class="input-icon <#if !iconLeft>right</#if>">
...
<#macro dropdownInput label id options="" hint="" name="" value="" required=false disabled=false tooltip="" style="" filter=false onchange =""  emptyText="Please Select" allowEmpty=false dataLiveSearch=true isMultiple=false isUseLabels=false isClearable=true class="" dataAttribute="" labelColSize="col-sm-4" inputColSize="col-sm-7" isMakerChange=false>
<div class="form-group row" style="${style!}">
    <@labels label=label required=required labelColSize=labelColSize isMakerChange=isMakerChange/>
    <div class="${inputColSize}">
        <select class="ui dropdown <#if dataLiveSearch>search</#if> form-control fluid show-tick <#if tooltip?has_content>tooltips</#if>" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" <#if required>required</#if> <#if disabled>disabled</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if onchange?has_content> onchange="${onchange!}"</#if> <#if isMultiple>multiple=""</#if> <#if dataError?has_content>data-error="${dataError}"</#if> >
        <#if allowEmpty>
            <option value="">${emptyText!}</option>
        </#if>
        <#if options?has_content>
...
<#--  
<#macro dropdownInput label id options="" hint="" name="" value="" required=false disabled=false tooltip="" filter=false onchange =""  emptyText="Please Select" allowEmpty=false dataLiveSearch=true isMultiple=false isUseLabels=false isClearable=true class="" dataAttribute="" labelColSize="col-sm-4" inputColSize="col-sm-7" isMakerChange=false>
<div class="form-group row">
   <label class="${labelColSize} field-text <#if isMakerChange>bg-white</#if>" >${label} <#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="${inputColSize}">
      	<select class="ui dropdown <#if dataLiveSearch>search</#if> form-control <#if tooltip?has_content>tooltips</#if>" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" <#if required>required</#if> <#if disabled>disabled</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> onchange="${onchange!}" <#if isMultiple>multiple=""</#if> >
	  	<#if allowEmpty>
	  	<option value="">${emptyText!}</option>
...
<div class="form-group row <#if state?has_content>has-${state!}</#if>">
   <label class="col-sm-4 field-text <#if required>text-danger</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-sm-7">
...
<div class="form-group row <#if state?has_content>has-${state!}</#if>">
   <label class="col-sm-3 field-text <#if required>text-danger</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-sm-8">
      <textarea class="form-control <#if tooltip?has_content>tooltips</#if>" rows="${rows}" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" placeholder="${placeholder}" <#if required>required</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if dataError?has_content>data-error="${dataError}"</#if> <#if maxlength?has_content>maxlength="${maxlength!}"</#if>>${value!}</textarea>
      <div class="form-control-focus">
      </div>
...
<div class="form-group row">
   <label class="col-md-4 field-text" for="${id}">${label}</label>
   <div class="col-md-7">
...
<div class="form-group row">
   <label class="col-md-4 field-text" for="${idFrom}">${label}</label>
   <div class="col-md-7">
...
<#macro fromSimpleAction id="" cancelUrl="" cancelLabel="Cancel" submitLabel="Submit" iconClass="" isSubmitAction=true showCancelBtn=true showSubmitBtn=true actionClass="">
<div class="clearfix"> </div>
<div class="col-md-1 col-sm-1 pl-0 <#if actionClass?has_content>${actionClass!}</#if>">
     <#if showCancelBtn>
...
<button type="${type!}" class="${btnClass}" id="${id!}" name="<#if !name?has_content>${id}<#else>${name}</#if>"><#if iconClass?has_content><i class="${iconClass}"></i> </#if>${label}</button>
</#macro>
...
<div class="form-group row">
   <label class="col-sm-4 field-text <#if required>text-danger</#if> <#if labelClass?has_content>${labelClass}</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-sm-7">
...
<div class="form-group row <#if state?has_content>has-${state!}</#if>" <#if groupId?has_content>id="${groupId!}</#if>">
   <label  class="col-sm-4 field-text <#if required>text-danger</#if>" for="${id}">
     <h2 class="float-left field-text has-error">${label!}</h2>
   </label>
   <div class="col-sm-12">
...
<div class="col-md-2 col-sm-2">
   <div class="form-group <#if required>has-error</#if> ${id} ">
      <label class="field-text <#if required>text-danger</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
      <input type="${inputType}" class="form-control input-sm <#if tooltip?has_content>tooltips</#if> " value="${value!}" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" placeholder="${placeholder}" <#if disabled>disabled</#if> <#if readonly>readonly</#if> <#if required>required</#if> <#if dataError?has_content>data-error="${dataError}"</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if inputType=="number" && min?has_content> min="${min}" </#if> >
      <#if disabled>
      <input type="hidden" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" value="${value!}"/>	
      </#if>
...
<div class="col-md-2 col-sm-2">
   <div class="form-group <#if required>has-error</#if> ${id}">
      <label class="field-text" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
...
<#macro displayRow label value="" isTag=false required=false disabled=false>
	<div class="form-group row">
		<label  class="col-sm-4 field-text <#if required>text-danger</#if>">${label!}</label>
		<div class="col-sm-7">
			<label class="field-text input-sm fw"><#if isTag>[${value!}]<#else>${value!}</#if></label>
...
<div class="form-group row">
  <label  class="col-sm-4 field-text has-error <#if required>text-danger</#if>">${label!}</label>
  <div class="col-sm-7">
...
<#macro radioButtonInput label id name options value="" radioInline=false required=false>
<div class="form-group row">
   <label class="col-sm-4 field-text ">${label}<#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-sm-7">
         <#list options.entrySet() as entry>  
         <div class="form-check-inline">
            <label for="${entry.key}">
            <input type="radio" id="${id}" name="${name}" value="${entry.key!}" class="form-check-input" <#if value?has_content && entry.key == value>checked</#if> >
            <span></span>
            <span class="check"></span>
            <span class="box"></span>
            ${entry.value!}</label>
...
<#macro generalInputModal label id name="" modalName="" value="" placeholder="" iconClass="" inputType="text" dataError="" tooltip="" disabled=false readonly=false required=false  min="" max="" maxlength="" pattern="" hint="" labelColSize="col-sm-4">
<div class="form-group row">
   <label class="${labelColSize} field-text" >${label} <#if required><span class="text-danger">&#42;</span></#if></label>
   <div class="col-sm-7">
...
<div class="form-group row">
   <label class="col-sm-4 field-text <#if required  || mandatory>text-danger</#if>" for="${colId1}">${label}<#if required><span class="text-danger">&#42;</span></#if><#if mandatory><span class="text-danger">&#94;</span></#if></label>
   <div class="col-sm-2"> 
...
<#macro mobileInput label id name="" value="" labelClass="col-5 field-text" placeholder="" iconClass="" inputType="text" dataError="" tooltip="" iconLeft=true disabled=false readonly=false required=false  min="" max="" maxlength="" pattern="" mandatory=false onkeyup="">
<div class="form-group row">
...
<div class="form-group row <#if required>has-error</#if> ${id} ">
   <label class="col-5 field-text <#if required || mandatory>text-danger</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if><#if mandatory><span class="text-danger">&#94;</span></#if></label>
   <div class="col-7">
...
<div class="form-group row">
   <label class="col-5 field-text <#if required || mandatory>text-danger</#if>" for="${colId1}">${label}<#if required><span class="text-danger">&#42;</span></#if><#if mandatory><span class="text-danger">&#94;</span></#if></label>
...
<div class="form-group row <#-- <#if required>has-error</#if> --> ${id} ">
   <label class="col-sm-4 field-text <#if required || mandatory>text-danger</#if>" for="${id}">${label}<#if required><span class="text-danger">&#42;</span></#if><#if mandatory><span class="text-danger">&#94;</span></#if></label>
...
<#macro menuItemLink getMenuGV short>
			
	<#assign shortcutList = getMenuGV.get("shortcut#"+short.shortcutId)?if_exists>
	
  	<#if !shortcutList?has_content>
	<li><a href="${short.requestUri?if_exists}">${uiLabelMap.get(short.uiLabels)}</a></li>
	<#else>
	<li>
		<a href="#${short.shortcutId}" data-toggle="collapse" aria-expanded="false"
		class="dropdown-toggle">${uiLabelMap.get(short.uiLabels)}</a>
		<ul class="collapse list-unstyled" id="${short.shortcutId}">
			<#list shortcutList as sc>
			<@menuItemLink getMenuGV sc />
			</#list>
		</ul>
	</li>
	</#if>
...
<#macro menuItemLinkActive getMenuGV short shortcutId="">
	...
	<li>
		<a href="#${short.shortcutId}" data-toggle="collapse" aria-expanded="false"
		class="dropdown-toggle">${uiLabelMap.get(short.uiLabels)}</a>
		<ul class="collapse list-unstyled <#if shortcutId?exists && shortcutId?has_content && shortcutId == "${short.shortcutId!}">show</#if>" id="${short.shortcutId}">
			<#list shortcutList as sc>
			<@menuItemLinkActive getMenuGV sc shortcutId/>
			</#list>
		</ul>
	</li>
	</#if>
...
<#macro generalSearchInput id label="" name="" value="" labelClass="col-sm-4 field-text" hint="" placeholder="" iconClass="" inputType="text" dataError="" tooltip="" iconLeft=true disabled=false readonly=false required=false min="" max="" step="" maxlength="" minlength="" pattern="" mandatory=false onkeyup="" labelColSize="col-sm-4" inputColSize="col-sm-7" searchTarget="" lookupParams="" isMakerChange=false>
<div class="<#if label?has_content>form-group row<#else>form-group</#if>">
   <#if label?has_content> <label for="<#if !name?has_content>${id}<#else>${name}</#if>" class="${labelColSize} field-text <#if isMakerChange>bg-white</#if>">${label} <#if required><span class="text-danger">&#42;</span></#if></label></#if>
    <div class="${inputColSize}">
       <div class="input-group">
          <input type="${inputType}" class="form-control input-sm <#if tooltip?has_content>tooltips</#if> <#if inputType=="number">no-spin</#if>" autocomplete="off" value="${value!}" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" placeholder="${placeholder}" <#if disabled>disabled</#if> <#if readonly>readonly</#if> <#if required>required</#if> <#if dataError?has_content>data-error="${dataError}"</#if> <#if tooltip?has_content>data-original-title="${tooltip}"</#if> <#if onkeyup?has_content>onkeyup="${onkeyup}"</#if> <#if inputType=="number" && min?has_content> min="${min}" </#if> minlength="${minlength!}" maxlength="${maxlength!}" <#if inputType == "email">pattern="^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\])+\.([A-Za-z]{2,4})$" <#else><#if pattern?has_content>pattern="${pattern!}"</#if></#if> <#if step?has_content>step="${step!}"</#if>>
          <span class="input-group-addon"><span class="fa fa-search" data-toggle="modal" data-target="#${searchTarget!}" id="lookup_${id}"
	      	<#if lookupParams?has_content>
	      		<#list lookupParams?keys as key>
	      			data-${key}="${lookupParams[key]}" 
	      		</#list>
...
<#macro searchDropdown id options="" hint="" name="" value="" required=false disabled=false tooltip="" filter=false onchange =""  emptyText="Please Select" allowEmpty=false dataLiveSearch=true valueKeyPair=false class="" dataAttribute="" labelColSize="col-sm-4" inputColSize="col-sm-7">
<div class="form-group">
	<select class="ui dropdown <#if dataLiveSearch>search</#if> form-control fluid show-tick <#if tooltip?has_content>tooltips</#if>" id="${id}" name="<#if !name?has_content>${id}<#else>${name}</#if>" <#if required>required</#if> <#if disabled>disabled</#if> <#if dataLiveSearch> data-live-search="true"</#if>  <#if tooltip?has_content>data-original-title="${tooltip}"</#if> onchange="${onchange!}"  >
	<#if allowEmpty>
		<option value="">${emptyText!}</option>
	</#if>
	<#if options?has_content>
		<#list options.entrySet() as entry>  
			<option value="${entry.key}" <#if value?has_content && value == entry.key>selected</#if> >${entry.value!}<#if valueKeyPair> (${entry.key!})</#if></option>
		</#list>
	</#if>
	</select>
	<div class="help-block with-errors" id="${id}_error">${hint!}</div>
...
<#macro sectionFrameHeader title extra="" extraLeft="">
...
<#macro formButton btn1=true btn1label="Submit" btn1type="button" btn2type="" btn3type="" btn2=false btn2label="" btn3=false btn3label="" btn1onclick="" btn2onclick="" btn3onclick="">
<div class="text-left ml-1">
   <#if btn1><button type="${btn1type}" class="btn btn-sm btn-primary" <#if btn1onclick?has_content>onclick="${btn1onclick!}" </#if> > ${btn1label}</button></#if>
   <#if btn2><button type="${btn2type}" class="btn btn-sm btn-secondary" <#if btn2onclick?has_content>onclick="${btn2onclick!}" </#if> > ${btn2label}</button></#if>
   <#if btn3><button type="${btn3type}" class="btn btn-sm btn-secondary" <#if btn3onclick?has_content>onclick="${btn3onclick!}" </#if> > ${btn3label}</button></#if>
</div>
</#macro>
...
<#macro labels label required labelColSize="col-sm-4" isMakerChange=false>
    <label class="${labelColSize} field-text <#if isMakerChange>bg-light</#if>" >${label} <#if required><span class="text-danger">&#42;</span></#if></label>
</#macro>