...
package org.fio.admin.portal.login;
...
public class LoginEvents {
    ...
    public static String emailPassword(HttpServletRequest request, HttpServletResponse response) {
        String defaultScreenLocation = "component://admin-portal/widget/user-mgmt/UserMgmtScreens.xml#PasswordEmail";
        ...
        String passwordScreenLocation = request.getParameter("passwordScreenLocation");
        if(UtilValidate.isNotEmpty(passwordScreenLocation)) {
        	defaultScreenLocation = passwordScreenLocation;
        }
        ...
        try {
            ...
            if (useEncryption) {
                ...
                passwordToSend = org.fio.homeapps.util.DataUtil.generateSecurePassword(Integer.parseInt(EntityUtilProperties.getPropertyValue("security", "password.length.min", "5", delegator)));
                ...
                autoPassword = org.fio.homeapps.util.DataUtil.generateSecurePassword(Integer.parseInt(EntityUtilProperties.getPropertyValue("security", "password.length.min", "5", delegator)));
		...
		try {
			...
			redirectUrl = protocal+"://"+host+ ((UtilValidate.isNotEmpty(port) && port > 0) ? ":"+port : "");
        ...
        bodyParameters.put("redirectUrl", redirectUrl);        
        ...
        */
        errMsg = UtilProperties.getMessage(resource, "loginevents.password_reset_link_sent", UtilHttp.getLocale(request));
        request.setAttribute("_EVENT_MESSAGE_", errMsg);
        return "success";
    }
