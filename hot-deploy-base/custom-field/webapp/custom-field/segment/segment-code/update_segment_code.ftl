...
<div class="row">
	...
	<#assign extra='<a href="viewSegmentValueForGroup?groupId=${customFieldGroup.groupId}" class="btn btn-xs btn-primary m5 tooltips" title="View Segment Values for ${customFieldGroup.groupName!}" >View Segment Values</a>'/>
		<#assign extra=extra+'<a href="segmentValueForGroup?groupId=${customFieldGroup.groupId}" class="btn btn-xs btn-primary m5 tooltips" title="Add Segment Values for ${customFieldGroup.groupName!}" >Add Segment Values</a>'/>
		<#if customFieldGroup.isActive?has_content && customFieldGroup.isActive == "Y" >
			<#assign extra=extra+'<a href="#" class="btn btn-xs btn-primary m5 tooltips active-btn" title="Inactive Segment Code" data-isActive="N">Inactive</a>'/>
		<#else>
			<#assign extra=extra+'<a href="#" class="btn btn-xs btn-primary m5 tooltips active-btn" title="Active Segment Code" data-isActive="Y">Active</a>'/>
		</#if>
		 

	<div class="col-lg-12 col-md-12 col-sm-12 dash-panel"> 
	<@sectionFrameHeaderTab title="${uiLabelMap.Update} ${uiLabelMap.SegmentCode} [ ${customFieldGroup.groupName} ]" extra=extra tabId="EditSegmentCode"/>
	 <div class="row padding-r" >
		<div class="col-lg-6 col-md-6 col-sm-12">		
		<div class="portlet-body form">
			<form id="mainFrom" role="form" class="form-horizontal" action="<@ofbizUrl>updateSegmentCode</@ofbizUrl>" encType="multipart/form-data" method="post" data-toggle="validator">
			...
			<@dropdownCell 
				...
				required=true
				value=customFieldGroup.valueOverrideType
				allowEmpty=true
				dataLiveSearch=true
				placeholder=uiLabelMap.pleaseSelect
		...
		<div class="form-group row">
                    <div class="offset-sm-4 col-sm-7">
                      <@submit label="Submit"/>
					  <@cancel label="Cancel" onclick="findSegmentCode"/>
                    </div>
                </div>
...
function onSubmitRoleValidation() {
	valid = true;
	if ($('#roleTypeId').val() == "") {
