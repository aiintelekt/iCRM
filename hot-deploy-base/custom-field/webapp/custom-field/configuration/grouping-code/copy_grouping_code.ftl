...
<#assign extra='<a href="#"  class="btn btn-xs btn-primary back-btn" onclick="window.history.back();"><i class="fa fa-chevron-circle-left" aria-hidden="true"></i> Back</a>'/>
<div class="row" id="update-code">
	<div id="main" role="main">
		<form id="mainFrom" method="post" action="<@ofbizUrl>copyGroupingCodeAction</@ofbizUrl>" data-toggle="validator">
			<input type="hidden" name="parentGroupingCodeId" value="${groupingCodeId!}"/>	
			<input type="hidden" name="groupType" value="${groupingCode.groupType!}"/>
			<div class="col-lg-12 col-md-12 col-sm-12">
				<@sectionFrameHeader title="${uiLabelMap.Copy} ${uiLabelMap.GroupingCode} [ <small>${groupingCode.groupType!}</small> ]" extra=extra/>
			</div>
			<br>
			...
			<div class="offset-md-2 col-sm-12">
				...
				<@cancel
					label="Cancel"
					onclick="editGroupingCode?groupingCodeId=${groupingCodeId!}"
					/>
			</div>
		</form>
	</div>
