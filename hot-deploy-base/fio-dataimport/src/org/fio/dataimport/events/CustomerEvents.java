...
public class CustomerEvents {
    ...
    public static String module = CustomerEvents.class.getName();

    public static Map<String, Object> importCustomers(DispatchContext dctx, Map<String, ?> context) {
        int imported = 0;
        OpentapsImporter customerImporter = null;
        try {
            customerImporter = new OpentapsImporter("DataImportCustomer", dctx, new CustomerDecoder(context));
            imported += customerImporter.runImport(context);
        } catch (GeneralException e) {
            return UtilMessage.createAndLogServiceError(e, module);
        }
        Map<String, Object> result = ServiceUtil.returnSuccess();
        result.put("customersImported", imported);
        result.put("importedDataList", customerImporter.importedDataList);
        return result;
    }
    
    public static Map<String, Object> importContacts(DispatchContext dctx, Map<String, ?> context) {
        int imported = 0;
        OpentapsImporter contactImporter = null;
        try {
            contactImporter = new OpentapsImporter("DataImportContact", dctx, new ContactDecoder(context));
            imported += contactImporter.runImport(context);
        } catch (GeneralException e) {
            return UtilMessage.createAndLogServiceError(e, module);
        }
        Map<String, Object> result = ServiceUtil.returnSuccess();
        result.put("contactsImported", imported);
        result.put("importedDataList", contactImporter.importedDataList);
        return result;
    }
    ...
    public static Map<String, Object> importCustomersCommissions(DispatchContext dctx, Map<String, ?> context) {
        ...
        try {
            ...
            imported += commissionRatesImporter.runImport(context);
