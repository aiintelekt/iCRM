   ...
   <div class="row" id="note-info"  style="width:100%">
      <div class="col-md-12 col-lg-12 col-sm-12 dash-panel">
         <#if componentUri?has_content && domainEntityType == "SERVICE_REQUEST">
	         <#assign extra='<a href="${componentUri!}/viewServiceRequest?srNumber=${partyId!}#sr-notes" class="btn btn-xs btn-primary">
	     ...
	     <#elseif domainEntityType == "REBATE">
	         <#assign extra='<a href="/rebate-portal/control/viewRebate?agreementId=${domainEntityId!}#a-notes" class="btn btn-xs btn-primary">
	         <i class="fa fa-chevron-circle-left" aria-hidden="true"></i> Back</a>' />
	         <@sectionFrameHeader title="Notes : Information" extra=extra />
         </#if>
         
         <#if componentUri?has_content>
         <#assign linkedFrom = "${componentUri}/viewServiceRequest?srNumber=${partyId!}#sr-notes" />
         ...
         <@headerH2
         title="${requestParameters.noteId?if_exists} [${requestParameters.noteName?if_exists}]"
         />
         <div class="pt-2">
            <@headerH1
            title="Notes"
