...
public class OpportunityEscalationResolver extends Resolver {
	private static final String MODULE = OpportunityEscalationResolver.class.getName();

	private static OpportunityEscalationResolver instance;
	
	public static synchronized OpportunityEscalationResolver getInstance(){
        if(instance == null) {
            instance = new OpportunityEscalationResolver();
        }
        return instance;
    }
	@Override
	protected Map<String, Object> doResolve(Map<String, Object> context) throws Exception {
		...
		try {
			...
			if (UtilValidate.isNotEmpty(context)) {
				...
				String opportunityTypeId = ParamUtil.getString(context, "opportunityTypeId");
				...
				conditionList.add(EntityCondition.makeCondition(EntityOperator.AND,
						EntityCondition.makeCondition("opportunityTypeId", EntityOperator.EQUALS, opportunityTypeId),
						EntityCondition.makeCondition("status", EntityOperator.EQUALS, "ACTIVE")
		                ));
				/*
				if(UtilValidate.isNotEmpty(domainEntityType))
					conditionList.add(EntityCondition.makeCondition("opportunityAssocId", EntityOperator.EQUALS, domainEntityType));
				*/
				EntityCondition mainConditons = EntityCondition.makeCondition(conditionList, EntityOperator.AND);
				GenericValue slaConfig = EntityQuery.use(delegator).from("OpportunitySlaConfig").where(mainConditons).queryFirst();
	...
	public Map<String, Object> slaCalDate(Map<String, Object> context){
