			...
			if (UtilValidate.isNotEmpty(orderId)) {
        		...
        		if (UtilValidate.isNotEmpty(itms)) {
            		...
            		if (UtilValidate.isNotEmpty(templateId)) {
            			...
            			toEmail = org.fio.homeapps.util.PartyHelper.getEmailAddress(delegator, receipt.getString("partyId"), "PRIMARY_EMAIL");
    					...
    					rc.put("orderId", orderId);
    					rc.put("storeId", receipt.getString("criteriaId"));
            		...
            		String contactListId = UtilCampaign.getFirstCampaignContactListId(delegator, UtilMisc.toMap("marketingCampaignId", campaignId, "contactPurposeType", "LIVE", "isActive", "Y"));
            		
            		int randomInt = Math.abs(new Random().nextInt(500));
            		Timestamp fromDate = UtilDateTime.addSecondsToTimestamp(UtilDateTime.nowTimestamp(), randomInt);
            		
            		GenericValue cclp = delegator.makeValue("CampaignContactListParty");
            		cclp.put("contactListId", contactListId);
            		cclp.put("partyId", receipt.getString("partyId"));
            		cclp.put("fromDate", fromDate);
            		cclp.put("contactPurposeTypeId", "LIVE");
            		cclp.put("uniqueMailKey", null);
            		cclp.put("emailAddress", toEmail);
            		cclp.put("preferredContactMechId", null);
            		cclp.put("contactListSeqId", UtilGenerator.getNextSeqId());
            		delegator.createOrStore(cclp);
