package org.groupfio.common.portal.reporting;
...
import org.groupfio.common.portal.reporting.jasper.JRResourceBundle;
...
public final class UtilReports {
    ...
    //public static DefaultJasperReportsContext jrContext = null;
    static {
    ...
    public static List<GenericValue> getJRSupportedMimeType(Delegator delegator) throws GenericEntityException {
        ...
        return delegator.findList("MimeType", EntityCondition.makeCondition("mimeTypeId", EntityOperator.IN, supportedTypes), null, UtilMisc.toList("description"), null, false);
    }
            ...
            if (UtilValidate.isEmpty(report)) {
                ...
                if (UtilValidate.isEmpty(report) && UtilValidate.isNotEmpty(design)) {
                    if (reportLastModified > 0) {
                        report = (JasperReport) JRLoader.loadObject(new File(baseLocation));
                    } else {
    ...
    public static List<Map<String, Object>> getManagedReports(String componentName, String reportGroupId, Delegator delegator, Locale locale) {
        try {
            ...
            List<GenericValue> applicationGroups = delegator.findList("ReportGroup", EntityCondition.makeCondition(conditions, EntityOperator.AND), null, null, null , false);
            ...
            for (GenericValue group : applicationGroups) {
                ...
                members = delegator.findByAnd("ReportGroupMemberRegistry", UtilMisc.toMap("reportGroupId", group.getString("reportGroupId")), null, false);
        ...
        try {
            GenericValue transformation = EntityUtil.getFirst(
                    delegator.findByAnd("DataWarehouseTransform",
                            UtilMisc.toMap("transformEnumId", enumId), UtilMisc.toList("transformTimestamp DESC"), false
                    )
            );
